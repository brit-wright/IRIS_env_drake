...
def connectsTo(tens_start, tens_goal, starts, goals):
    ...
    for b in range(num_divisions):
        ...
        all_points_non_vert = torch.stack([x_vals_non_vert, y_vals_non_vert], dim=2) # will have shape (num_divisions, num_paths, 2)

        new = all_points_non_vert[b] # shape will be (num_paths, 2)

        #### NOTE: Come back here lol
        
        # print(f'from connects to')
        result, timed = inFreespace(new, starts, goals)
        counting_infreespace += 1
        timing_infreespace += timed
    ...
    if len(vertical_indices > 0):
        ...
        for b in range(num_divisions):
            ...
            all_points_vert = torch.stack([x_vals_vert, y_vals_vert], dim=2) # will have shape (num_divisions, num_paths, 2)

            new = all_points_vert[b] # shape will be (num_paths, 2)
